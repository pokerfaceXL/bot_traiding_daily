...
Tests the hypothesis in spec/research/F006-hypothesis-notrail-monthly-catalog5.md: whether
any of the 50 (name, symbol, interval) series -- EMA3_21_50_200, EMA3_13_50_200,
BB_20_2_EMA200, EMA_50_200, BB_20_25_EMA200 x 5 symbols x 2 intervals -- clears the frozen
protocol's monthly promotion checklist (spec/research/F005-validation-protocol.md section 7)
at the NO_TRAIL control (activate_pct=10.0, trail never arms) on Train 1.

Adapted directly from scripts/f006_notrail_monthly_experiment.py (same structure), with the
NAMES list swapped to the 5 catalog-sweep names, the harness control diffed against
output/f006_catalog_notrail_sweep/summary/results.csv instead, and promotion_pass extended
with an explicit n_trades > 0 guard.
...
Writes only output/f006_notrail_monthly_catalog5/. Zero network connections.
"""
...
INTERVALS = ["240", "60"]
NAMES = ["EMA3_21_50_200", "EMA3_13_50_200", "BB_20_2_EMA200", "EMA_50_200", "BB_20_25_EMA200"]
...
# NO_TRAIL cell, identical to every prior F006 NO_TRAIL script.
ACTIVATE_PCT = 10.0
...
OUT_DIR = "output/f006_notrail_monthly_catalog5"
PRIOR_RESULTS = "output/f006_catalog_notrail_sweep/summary/results.csv"
...
def _run_one(train1_df, mask, symbol, interval, strategy_name) -> dict:
    ...
    # regularity.classify_days() unconditionally flags the very first day of any
    # equity curve as "missing" (no prior day to diff pnl against -- see
    # regularity.py's classify_days docstring/comment), which silently drops that
    # day's real equity delta from every _net_pnl_for_* sum below. Harmless for the
    # 3 original NO_TRAIL leads (rolling-window Donchian/BB strategies need their
    # full lookback window before any signal, so day 1 never trades), but the EMA
    # ewm()-based names in this sample have no such warm-up requirement and can
    # trade on day 1. That day falls inside the warm-up months (2024-01/02, never
    # evaluated for promotion either way), but must still be captured for the
    # harness reconciliation against the catalog sweep's stored full-slice totals.
...
def main():
    ...
    if os.path.exists(PRIOR_RESULTS):
        ...
        prior_no_trail = prior[
            (prior["no_trail"] == True) & (prior["mask_mode"] == "one_shot")  # noqa: E712
            & (prior["strategy"].isin(NAMES))
        ]
        ...
        for _, r in merged.iterrows():
            ...
            # (warm-up buffer + Train 1 + Warsaw-local boundary day), since that
            # script never sliced by calendar month; this slice's train1_net_pnl
            # deliberately excludes the 2024-01/02 warm-up buffer and the boundary
            # day (neither is evaluated by the protocol), so the invariant checked
            # here is train1 + warmup + boundary == prior's full total.
            reconstructed = (
                r["train1_net_pnl"] + r["warmup_net_pnl"] + r["boundary_net_pnl"]
                + r["first_day_net_pnl"]
            )
            if abs(reconstructed - r["net_pnl"]) > 1e-3 or r["n_trades"] != r["n_trades_prior"]:
                mismatches.append({
                    ...
                    "boundary_net_pnl": r["boundary_net_pnl"],
                    "first_day_net_pnl": r["first_day_net_pnl"],
        ...
        if len(merged) != 50:
            raise SystemExit(
                f"STOP: expected 50 rows to compare against {PRIOR_RESULTS}, got {len(merged)}."
            )
        if mismatches:
            raise SystemExit(
        ...
        raise SystemExit(f"STOP: {PRIOR_RESULTS} not found -- cannot run harness control.")

    passing = [r for r in rows if r["promotion_pass"]]
    zero_trade_series = [
        {"symbol": r["symbol"], "interval": r["interval"], "strategy": r["strategy"]}
        for r in rows if r["n_trades"] == 0
    ]

    manifest_out = {
        "script": "scripts/f006_notrail_monthly_catalog5_experiment.py",
        ...
        "n_series_zero_trades": len(zero_trade_series),
        "zero_trade_series": zero_trade_series,
    ...
    print(f"zero-trade series: {len(zero_trade_series)}/{len(rows)}")
