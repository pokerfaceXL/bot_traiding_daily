...
- 🟡 IN PROGRESS — F006-strategy-research: porównanie hipotez i wybór finalistów na danych rozwojowych; po F005 i F010. Diagnostyka dźwigni/cooldown (F005) i pozytywna kontrola pipeline'u (`F006-pipeline-positive-control.md`) potwierdziły: przewaga istniejącego katalogu jest ujemna niezależnie od sizing'u, nie to jest błąd silnika. Hipotezy 1-2 (szerokość SL; filtr reżimu ADX14 na wejściu) odrzucone — żadna poprawia net PnL ponad szum, 0 dodatnich przebiegów w obu. Dowody: `F006-hypothesis-stop-width.md`, `F006-hypothesis-entry-regime-filter.md`. Hipoteza 3 (Lorentzian Classification, advanced-ta 0.1.8): adapter zbudowany i uczyniony przyczynowo poprawnym (biblioteka domyślnie przecieka przyszłość przez normalizację na całej ramce i indeksowanie maxBarsBack; naprawione oknem narastającym) — na ustawieniach domyślnych **gorszy niż katalog na 10/10 serii Train 1** (mean PnL -$384.99 vs -$357.81, DD 77% vs 72%, przetrwało 2/10 vs 27/100), jedyna przewaga to +1.2pp win rate w granicach szumu próby. Dowód: `spec/research/F006-lorentzian-causality.md`. Hipoteza 4 (cooldown_candles przy leverage=1, 600 przebiegów, katalog+Lorentzian): cooldown 0→50 redukuje stratę o 41% (-$363.67→-$215.40) i DD (73%→43%, survived 29→ 98/120) monotonicznie u 11/12 sygnałów, ale **przez handel rzadziej, nie przez lepsze transakcje** — PnL/transakcję przy 4h praktycznie stałe (-$1.260→-$1.266) mimo 61% mniej transakcji, ujemna oczekiwana wartość w 600/600 przebiegach. 0/600 dodatnich. Dowód: `spec/research/F006-hypothesis-cooldown.md`. Hipoteza 5 (one-shot flip entry, `entry_regime_mask` — jeden wpis na ciągły ‛call” sygnału, bez zmian silnika): mechanizm potwierdzony — poprawia PnL/transakcję o 8.6% (Lorentzian 4h: 4.46 wejść/call → 1.00, -$382→-$79) i win rate +2.4pp, czego cooldown nie robił, ale całkowity PnL (+$98 vs +$148 dla cooldown=50) nie pobija cooldownu — odrzucona wobec zadeklarowanego warunku. Dowód: `spec/research/F006-hypothesis-one-shot-entry.md`. Pięć niezależnych hipotez (SL/ADX/cooldown/Lorentzian-domyślny/one-shot) zgodnych: ujemna oczekiwana wartość na transakcję jest właściwością sygnałów, nie silnika/sizing'u/kosztu/momentu wejścia. Następny krok: nowa rodzina generatora (Donchian breakout/pullback), oceniana od razu z włączoną maska one-shot (tania i poprawiająca expectancy).
