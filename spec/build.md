...
- 🟡 IN PROGRESS — F006-strategy-research: porównanie hipotez i wybór finalistów na danych rozwojowych; po F005 i F010. Jedenaście hipotez wyczerpanych na starym katalogu i dwóch nowych generatorach (SL, ADX, cooldown, one-shot flip entry, Lorentzian Classification, Donchian breakout/pullback) — zero dodatnich do (6). Dowody: spec/research/F006-hypothesis-{stop-width,entry-regime-filter,cooldown,one-shot-entry,donchian}.md, F006-lorentzian-causality.md.
- **Przełom na geometrii wyjścia (hip. 7-8, 1440 przebiegów)**: sweep activate_pct×trail_pct + rozszerzenie do a=0.20/t=0.08 + kontrola bez trailingu — breakeven win rate 58.44%→42.90%→33.32%, **brak trailingu bije każdą z 18 przebadanych komórek trailingu** (breakeven 30.14%, pierwszy dodatni **brutto** wynik na transakcję w całym F006: +$0.19), 3 nazwy dodatnie zagregowane na Train 1 przy braku trailingu (`DONCHIAN_55`, `BB_20_25_breakout`, `DONCHIAN_PULLBACK_55`). Dowody: spec/research/F006-hypothesis-{trailing-sweep,trailing-boundary}.md.
- **Oś geometrii wyjścia + entry filtry wyczerpane dla tych 3 nazw na kryterium miesięcznym (hip. 9-12, ~750 przebiegów)**: miesięczny PnL — 0/30 serii przechodzi (każda ≥2 miesiące stratne na 12, straty rozłożone równomiernie). Szerszy SL (0.03→0.08 bez trailingu) poprawia net/trade +11.9% ale nie zamyka luki. Dwa niezależne filtry wejścia (brama ekspansji szerokości kanału/wstęgi; brama zgodności kierunku z trendem EMA50/EMA200) obie obalone — pierwsza „głodzi” transakcje (win rate +0.4pp przy -52% transakcji), druga aktywnie usuwa świeże wybicia, które są źródłem przewagi (win rate **-3.16pp**, każda z 3 nazw pogarsza się). Trzecia, niezależna próba na osi wielkości pozycji (sizing odwrotny do ATR%, nowy hook `stake_series` w `backtest_engine.py`, addytywny/domyślnie bez zmiany zachowania) również obalona: multiplikator nie różnicuje wygranych/przegranych (-0.0061pp, zły kierunek), 9/360 miesięcy-serii odwraca znak ale żadna seria nie czyści całej listy. Dowody: spec/research/F006-hypothesis-{notrail-monthly,stop-width-notrail,entry-width-expansion,entry-trend-confirm,position-sizing-vol-inverse}.md. Trzy niezależne mechanizmy oparte na wielkości/zmienności wszystkie zawodzą tym samym powodem (przewaga tkwi w świeżych wybiciach przy podwyższonej zmienności, które każdy miernik wielkości karze). **Hipoteza 14 (zgodność kierunku między symbolami, oś ortogonalna do zmienności)** — pierwszy mechanizm poruszający win rate we właściwym kierunku (+1.84pp trade-weighted), ale pomaga 2/3 nazwom (`BB_20_25_breakout`, `DONCHIAN_55`: win rate +2-6pp, PnL utrzymany/lepszy) i szkodzi trzeciej (`DONCHIAN_PULLBACK_55`: PnL dodatni→ujemny) — efekt netto się znosi, 0/30 czyści kryterium miesięczne, ale najlepsze serie mają już tylko 3 miesiące stratne (lepiej niż kiedykolwiek wcześniej w tej rodzinie). Dowód: spec/research/F006-hypothesis-entry-cross-symbol-agreement.md. **Dopracowanie (2 nazwy + sweep MIN_AGREE 1-4, 80 komórek)**: MIN_AGREE=2 potwierdzony jako najlepszy pojedynczy próg (win rate +2.82pp, mean PnL +$5.88/serię); MIN_AGREE=4 daje najbliższe dotąd podejście w całej rodzinie (2 miesiące stratne na 12, wcześniej najlepiej 3) ale na 2-5 transakcjach/rok — zbyt cienka próbka żeby liczyć się jako kandydat (ten sam tryb „głodzenia”, który "n_trades>0" łapie tylko przy zerze). 0/80 czyści kryterium. Dowód: spec/research/F006-hypothesis-entry-cross-symbol-refined.md. **Recency ostatniej straty (nowy hook `loss_cooldown_candles`, addytywny)**: jedyna dźwignia w całej rodzinie która **pogarsza** kryterium miesięczne (średnio 6.47→7.43 miesięcy stratnych), win rate płaski, PnL spada 42%. Mechanizm nie odróżnia „serii do przeczekania” od „następny sygnał to jedyny duży wygrywający tego miesiąca”. Dowód: spec/research/F006-hypothesis-loss-recency-cooldown.md. **Take-profit (ostatnia niepróbowana dźwignia, nowy hook `take_profit_multiple`, addytywny)**: TP=2×SL podnosi win rate +9.98pp ale ścina grube ogony wygranych — średni PnL na Train 1 **przechodzi na ujemny** (+$48.45→-$6.54, jedyny z 6 mechanizmów który to robi), najlepsza dotąd seria (`DONCHIAN_55`/DOGEUSDT/1h, 2 miesiące stratne) pogarsza się do 5. Dowód: spec/research/F006-hypothesis-exit-take-profit.md. Sześć niezależnych mechanizmów entry/exit/sizing wyczerpanych dla tych 3 nazw przy NO_TRAIL — tłuste ogony wygranych niezbędne do dodatniego PnL, każda próba ich obcięcia/przefiltrowania szkodzi. **Szerszy sweep katalogu przy NO_TRAIL (77 nowych nazw ze 85 w `STRATEGY_CATALOG`, ten sam koszyk/interwały)**: 54/77 (70%) zagregowanych dodatnich na Train 1 (vs 3/8 w oryginalnej próbce) — top 5 (`EMA3_21_50_200` +$125/serię, `EMA3_13_50_200`, `BB_20_2_EMA200`, `EMA_50_200`, `BB_20_25_EMA200`) bije wszystkie 3 oryginalne liderki na każdej mierze, głównie triple/dual EMA-crossover i BB+EMA200 (długi filtr trendu). Dowód: spec/research/F006-catalog-notrail-sweep.md. **Miesięczny PnL dla tych 5 nazw (50 serii)**: 0/50 czyści kryterium, najlepsza (`BB_20_25_EMA200`/SOLUSDT/1h) ma 3 miesiące stratne — lepiej niż średnia oryginalnej 3, ale **nie bije** najlepszego wyniku całej rodziny (`DONCHIAN_55`/DOGEUSDT/1h, 2 miesiące). **Łącznie 0/80 serii NO_TRAIL w obu notach czyści kryterium miesięczne.** Dowód: spec/research/F006-hypothesis-notrail-monthly-catalog5.md. **Wniosek: poszerzenie osi nazwy strategii znalazło szerszą przewagę zagregowaną, ale nie zmieniło kształtu „grudkowatych wygranych”, który odrzuca zero-tolerancyjna reguła miesięczna** — dotyczy każdej z 8 przebadanych nazw. **Eksploracja dywersyfikacji portfela (5 najlepszych serii, po jednej na symbol, sumowanie miesięczne pod wspólnym $500)**: portfel wyrównuje się do 2 miesięcy stratnych — tyle samo co najlepszy pojedynczy składnik, **bez poprawy** — bo w obu złych miesiącach większość składników była jednocześnie na minusie (skorelowane rodziny sygnału/momentum na skorelowanym koszyku krypto, nie tylko skorelowane aktywa). Nie zamyka F007, ale ta konkretna wersja dywersyfikacji nie jest warta wąskiej hipotezy F007. Dowód: spec/research/F006-portfolio-diversification-exploration.md. **Mean-reversion, jedyny mechanicznie inny rodzaj sygnału w katalogu (BB revert + RSI extreme, 5 nowych nazw + potwierdzenie RSI14_7030)**: wszystkie 5 zagregowane **ujemne** przy NO_TRAIL — ten koszyk trendował zbyt mocno na Train 1, żeby fade'owanie ekstremum się opłacało, przy dowolnym progu/lookbacku. To *cały* zestaw mechanicznie kontr-trendowych sygnałów w 85-nazwowym katalogu (potwierdzone czytaniem całego `strategy.py`) — gorszy tryb porażki niż „grudkowate wygrane” rodziny momentum (te przynajmniej mają realną przewagę zagregowaną). Dowód: spec/research/F006-hypothesis-mean-reversion-notrail.md. **Stan po 12 hipotezach**: 0/80 serii NO_TRAIL czyści kryterium miesięczne (momentum) + cała rodzina mean-reversion ujemna zagregowanie; najlepszy wynik całej rodziny to 2 miesiące stratne (`DONCHIAN_55`/DOGEUSDT/1h). Katalog wyczerpany na obu osiach mechanizmu; kolejny krok wymaga nowego źródła sygnału spoza `add_indicators` albo decyzji właścicielki o tolerancji/celu.
...
- 2026-09 (skrót) — F001 (kontrakt sygnału/ryzyka + 16 rozbieżności symulacja/live), F002 (`backtest_apex.py --local-csv` offline, kod wyjścia 1 na błąd), F003 (`data_contract.py` — zamknięte świece/luki/warm-up/manifest+checksuma), F004-execution-equity (`backtest_apex.py` liczy przez `backtest_engine.py` z pełnymi kosztami, 63 testy) — wszystkie scalone, szczegóły w spec/research/F00{1,2,3,4}-*.md. F005-validation-baseline + addendum dźwignia/cooldown: zamrożony protokół, 790+700 przebiegów na realnych danych Bybit — **0/24885 ważnych miesięcy spełnia cel, wszystkie kombinacje przekraczają DD>50% niezależnie od dźwigni/cooldownu**; przyczyna: leverage=10+margin=$100 → ~12-15 przegranych wyczerpuje $500. Dowody: `spec/research/F005-baseline.md`, `spec/research/F005-leverage-sensitivity.md`.
