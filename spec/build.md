...
- 🟡 IN PROGRESS — F006-strategy-research: porównanie hipotez i wybór finalistów na danych rozwojowych; po F005 i F010. Diagnostyka dźwigni/cooldown (patrz PROVEN, F005) pokazała: żadna wartość dźwigni/cooldownu nie ratuje istniejącego katalogu — przewaga sygnału jest ujemna niezależnie od sizing'u, to nie jest kwestia samej dźwigni. F006 może więc ruszyć (poszukiwanie NOWYCH hipotez, oddzielone od strojenia dźwigni per build.md), z leverage=1 jako konserwatywnym placeholderem badawczym — finalna decyzja o dźwigni/limitach ryzyka dla ewentualnego kandydata do produkcji nadal należy do właścicielki. Kontrola pozytywna (positive control) potwierdziła: pipeline poprawnie liczy dodatni wynik na sprzyjających danych (syntetyczny uptrend, +$32.58, deviation_pct=0.0% w lutym 2024) — 0/24885 i 0/700 to realne cechy istniejącego katalogu, nie błąd backtest_engine.py/costs.py/equity.py/execution.py/regularity.py. Dowód: spec/research/F006-pipeline-positive-control.md. Otwarte poszukiwanie hipotez ruszyło. Hipoteza 1 (szerokość SL): odrzucona w zbiorczej próbie (0/400 dodatnich na Train 1) — win rate rośnie monotonicznie z szerszym SL, ale net PnL nie, 3/10 strategii "floor-locked", 2/10 pogarsza się. Dowód: spec/research/F006-hypothesis-stop-width.md. Hipoteza 2 (filtr reżimu ADX14 na wejściu): również odrzucona w zbiorczej próbie (0/500 dodatnich na Train 1) — net PnL rośnie nieznacznie (~5% deficytu) i monotonicznie u 9/10 strategii, ale win rate zostaje płaski/szumowy; poprawa wynika z mniejszej liczby transakcji (mniejszy koszt), nie z lepszej selekcji wejść. Dowód: spec/research/F006-hypothesis-entry-regime-filter.md. Dwie niezależne hipotezy strojenia wejścia/wyjścia na istniejącym katalogu wyczerpane bez przywrócenia przewagi. Następny krok: nowy generator sygnału (Lorentzian Classification per priorytet F006 niżej, lub rodziny Donchian/pullback), nie kolejny filtr na istniejącym katalogu.
...
- 2026-09 — F005-validation-baseline: zamrożony protokół (spec/research/F005-validation-protocol.md) + zmierzony baseline (spec/research/F005-baseline.md) na realnych danych Bybit, 790 przebiegów (79 strategii × 5 symboli × 2 interwały). **Wynik: 0/24885 ważnych miesięcy spełnia cel, 790/790 przekracza DD>50%.** Zweryfikowana przyczyna (ręcznie sprawdzona na surowych transakcjach): leverage=10 + margin=stake=$100 → strata na SL ≈$30-33, ~12-15 przegranych wyczerpuje $500 poniżej progu $100 w 1-3 mies.
- 2026-09 — F005 addendum (dźwignia/cooldown): 700 dodatkowych przebiegów (leverage 1/2/3/5, cooldown 5/20 przy leverage=10, 10 strategii × 10 symbol/interwał) — **0/700 spełnia cel regularności, niższa dźwignia tylko spowalnia bankructwo konta (tygodnie→miesiące), nie zmienia jego kierunku** (margin=$100 niezależnie od dźwigni, zweryfikowane wprost w polu `margin` transakcji). Przewaga sygnału istniejącego katalogu jest ujemna niezależnie od sizing'u — to nie jest kwestia samej dźwigni. Dowód: `spec/research/F005-leverage-sensitivity.md`.
