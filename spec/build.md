...
- 🟡 IN PROGRESS — F006-strategy-research: porównanie hipotez i wybór finalistów na danych rozwojowych; po F005 i F010. Sześć niezależnych hipotez na Train 1, wszystkie odrzucone, 0 przebiegów dodatnich: (1) szerokość SL, (2) filtr reżimu ADX14, (3) Lorentzian Classification (adapter przyczynowo poprawny, advanced-ta domyślnie normalizuje na całej ramce — naprawione oknem narastającym — ale gorszy niż katalog na 10/10 serii), (4) cooldown_candles (redukuje stratę 41% przez rzadszy handel, PnL/transakcję bez zmian), (5) one-shot flip entry (entry_regime_mask, jeden wpis na ciągły sygnał — realny mechanizm, +8.6% PnL/transakcję, ale mniej niż cooldown=50 w sumie), (6) Donchian breakout/pullback (nowy generator, oceniony z maską one-shot — nadal 0/80 dodatnich). Dowody: spec/research/F006-hypothesis-{stop-width,entry-regime-filter,cooldown,one-shot-entry,donchian}.md, F006-lorentzian-causality.md. **Nowe ustalenie z (6), warte osobnej hipotezy**: rozkład na wygrane/przegrane pokazuje, że każda nazwa (stara i nowa) potrzebuje ~63% win rate na break-even, żadna nie przekracza 44% (średnia wygrana $1.46 vs średnia strata -$2.55) — 86% transakcji zamyka mechanika stopu/trailingu, nie sygnał; activate_pct/trail_pct są identyczne we wszystkich sześciu skryptach F006 i nigdy nie były przemiatane (tylko max_sl_pct w (1)). Następny krok: sweep activate_pct×trail_pct z włączoną maską one-shot, czytany pod kątem reward/risk, nie tylko net PnL — zanim uzna się rozszerzanie katalogu za wyczerpane.
