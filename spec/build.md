...
- 🟡 IN PROGRESS — F006-strategy-research: porównanie hipotez i wybór finalistów na danych rozwojowych; po F005 i F010. Dziewięć hipotez na entry/exit istniejącego katalogu i dwóch nowych generatorach (SL, ADX, cooldown, one-shot flip entry, Lorentzian Classification, Donchian breakout/pullback, exit-geometria) — Dowody: spec/research/F006-hypothesis-{stop-width,entry-regime-filter,cooldown,one-shot-entry,donchian}.md, F006-lorentzian-causality.md. **Hipotezy 7-8 (sweep activate_pct×trail_pct, potem rozszerzenie do a=0.20/t=0.08 + kontrola bez trailingu, 1440 przebiegów łącznie) — geometria wyjścia jest dźwignią**: breakeven win rate 58.44%→42.90%→33.32%, **brak trailingu bije każdą z 18 przebadanych komórek trailingu** (breakeven 30.14%, pierwszy dodatni **brutto** wynik na transakcję w całym F006: +$0.19). Dowody: spec/research/F006-hypothesis-{trailing-sweep,trailing-boundary}.md.
- **Hipotezy 9-10 (miesięczny PnL bez trailingu dla 3 zagregowanych dodatnich nazw + re-test szerokości SL bez trailingu, 350 przebiegów) — zamyka wątek bez kandydata**: 0/30 serii (`DONCHIAN_55`, `BB_20_25_breakout`, `DONCHIAN_PULLBACK_55` × 5 symboli × 2 interwały) przechodzi kryterium miesięczne — każda ma ≥2 miesiące stratne na 12 (straty rozłożone równomiernie, nie skoncentrowane). Szerszy SL (0.03→0.08) realnie poprawia net/trade (+11.9%, `avg_loser` wreszcie rośnie proporcjonalnie do `max_sl_pct`, wcześniej płaskie) ale nie zamyka luki do breakeven i nie zmienia wyniku miesięcznego. Dowody: spec/research/F006-hypothesis-{notrail-monthly,stop-width-notrail}.md. Wniosek: oś geometrii wyjścia (trailing + SL) wyczerpana dla tych 3 nazw. **Hipoteza 11 (filtr wejścia: brama ekspansji szerokości kanału/wstęgi, reużyta z `sig_bb_breakout_squeeze`)**: technicznie 1/30 serii przechodzi kryterium miesięczne, ale przez zero transakcji (pusta strategia trywialnie nie ma stratnego miesiąca) — w treści hipoteza obalona: win rate +0.42pp przy -52% transakcji, PnL spada, filtr “głodzi” zamiast selekcjonować (ten sam tryb porażki co stary filtr ADX). Dowód: spec/research/F006-hypothesis-entry-width-expansion.md. Luka w warunku obalenia (brak strażnika `n_trades>0`) odnotowana, nie ukryta. Oś geometrii wyjścia + jeden prosty filtr wejścia wyczerpane dla tych 3 nazw; następny krok wymaga filtra zmieniającego *które* wejścia są brane (np. potwierdzenie kierunkowe/multi-timeframe), nie samego progu zmienności, i musi mieć strażnika min. liczby transakcji w warunku obalenia.
